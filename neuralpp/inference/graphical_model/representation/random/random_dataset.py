...
def generate_assignment_dict(variables):
    ...
    assert all(
        isinstance(v, IntegerVariable) for v in variables
...
def generate_dataset_given_number_of_observation_and_query_variables(
    ...
    observed_variables = set(
        random.sample(model_variables, number_of_observed_variables)
    )
    query_variables = random.sample(
        list(model_variables - observed_variables), number_of_query_variables
    )
...
def generate_dataset_given_observation_and_query_variables(
    model,
    observed_variables,
    query_variables,
...
def generate_dataset_given_observation_dict_and_query_variables(
    datapoints_per_observation, model, observation_dict, query_variables
):
