...
def test_cache_by_id_on_loop():
    test_object_1 = MethodContainer()
    test_object_2 = MethodContainer()
    n = 10
    m = 5
    ...
    # Each call to the method should have been recorded.
    ...
    # These results should all be cached, so there are no direct calls to fm
    # so call_count should be unchanged.
    assert (test_object_1.call_count == n * n)
