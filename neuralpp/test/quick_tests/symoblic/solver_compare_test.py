...
def solve_inequalities_sympy(ineq_list: list) -> None:
    """
    SymPy's support for inequalities are a little hard to read. This is just a wrapper for inequalities.
    l is expected to be a list of (LHS, VARS, OP) where the inequality is "LHS OP 0", VARS contains all
...
def test_compare_z3_and_sympy_solver() -> None:
    """
...
def test_compare_z3_and_sympy_solver_sympy_fail() -> None:
    """
    SymPy's solve() targets a harder goal. So the scope of its solvable problems are smaller.
    It fails some simple tasks solvable in Z3. For example, it doesn't support >1 variables inequalities.
    """
    x, y = Ints('x y')
    s = Solver()
    ...
    # SymPy's solver is ill-equipped to solve the set of inequality.
    x, y = symbols('x y')
