...
def _repeat_n(
    operation: AbelianOperation, expression: Expression, size: Expression
) -> Optional[Expression]:
...
def _map_leaves_of_if_then_else(
    conditional_intervals: Expression, function: Callable[[Expression], Expression]
) -> Expression:
    """
    ...
    match conditional_intervals:
        case FunctionApplication(
            function=Constant(value=conditional), arguments=[if_, then, else_]
        ):
            return if_then_else(
                if_,
...
class Eliminator:
    ...
    def eliminate(
        ...
        def eliminate_at_leaves(dotted_interval: DottedIntervals) -> Expression:
            ...
            result = self._eliminate_interval(
                operation, index, dotted_interval.interval, body, is_integral, context
            )
            if not dotted_interval.dots:  # empty dots
                return result
            inverse = operation.inverse(
                reduce(operation, dotted_interval.dots, operation.identity)
            )
            return operation(result, inverse)

        try:
            with self.profiler.profile_section("from-constraint"):
                conditional_intervals = from_constraint(
                    index, constraint, context, is_integral, self.profiler
                )
            return _map_leaves_of_if_then_else(
                conditional_intervals, eliminate_at_leaves
            )
        except Exception as exc:
            raise RuntimeWarning(
                f"cannot eliminate {exc}."
            )  # delete this line to return a general result
            return BasicQuantifierExpression(
                operation, index, constraint, body, is_integral
            )

    def _eliminate_interval(
        self,
        operation: AbelianOperation,
        index: Variable,
        ...
        result = self._symbolically_eliminate(
            operation, index, interval, body, is_integral, context
        )
        if result is not None:
            return result

        if isinstance(interval.lower_bound, Constant) and isinstance(
            interval.upper_bound, Constant
        ):
        ...
        return BasicQuantifierExpression(
            operation, index, interval.to_context(index), body, is_integral
        )

    def _symbolically_eliminate(
        self,
        operation: AbelianOperation,
        index: Variable,
        interval: ClosedInterval,
        ...
        if operation.value == operator.add:
            if not is_integral:
                if (
                    result := Eliminator.symbolic_sum(body, index, interval)
                ) is not None:
                    return result
            else:
                if (
                    result := self.symbolic_integral(body, index, interval)
                ) is not None:
                    if context.is_known_to_imply(
                        interval.upper_bound >= interval.lower_bound
                    ):
                        return result
                    return if_then_else(
                        interval.upper_bound > interval.lower_bound, result, 0
                    )
    ...
    @staticmethod
    def symbolic_sum(
        body: Expression, index: Variable, interval: ClosedInterval
    ) -> Optional[Expression]:
        return SymPyExpression.symbolic_sum(
            body, index, interval.lower_bound, interval.upper_bound
        )

    def symbolic_integral(
        self, body: Expression, index: Variable, interval: ClosedInterval
    ) -> Optional[Expression]:
        ...
        with self.profiler.profile_section("integration"):
            if DRY_RUN:
                ...
                result = SymPyExpression.symbolic_integral(
                    body,
                    index,
