...
from neuralpp.inference.graphical_model.representation.random.multi_type_random_model_util import \
    ...
    make_random_table_factor, make_shifted_standard_gaussian_given_shift
...
def main():
    ...
    numbers_of_samples = \
        range(initial_number_of_samples, final_number_of_samples + number_of_samples_step, number_of_samples_step)
    ...
    types_of_variables_in_gaussians_in_switch = [TensorVariable] * cardinality_of_discrete_variables

    types_of_variables_in_switch_of_gaussians = \
        [TensorVariable, IntegerVariable] + types_of_variables_in_gaussians_in_switch
    ...
    def generate_original_factors():
        ...
        original_factors = \
            MultiTypeRandomModel(
                ...
                factor_makers=[
                    ...
                    FactorMaker(types_of_variables_in_switch_of_gaussians,
                ...
                from_type_to_variable_maker={
                    IntegerVariable: lambda name: IntegerVariable(name, cardinality_of_discrete_variables)
                },
        ...
        if use_simple_mixture_of_gaussians_instead:
            ...
            y = TensorVariable("y")
            components = [make_shifted_standard_gaussian_given_shift([y], distance_between_means * i)
                          for i in range(cardinality_of_discrete_variables)]
            original_factors = [
                PyTorchTableFactor([index], [1.0 / cardinality_of_discrete_variables] * cardinality_of_discrete_variables),
                SwitchFactor(index, components)
            ]
        return original_factors

    def make_model_variants_and_query(original_factors) -> Tuple[Tuple[Model, Model], Variable]:
    ...
    def get_estimate_and_time(model, query, number_of_samples):
        ...
        estimation_samples = solve_with_bm(
            model, query, variable_assignments, number_of_samples, number_of_chains)
        mean_estimate = samples_mean(estimation_samples)
        end = timer()
        time = end - start
        return mean_estimate, time

    data = list_for_each(numbers_of_samples,
                         lambda number_of_samples:

                         list_for_each(range(number_of_estimations),
                                       ...
                                       list_for_each(model_variants_and_queries_per_generated_model[0][0],
                                                     lambda model_variant:
                                                     get_estimate_and_time(
                                                         model=model_variant,
                                                         query=model_variants_and_queries_per_generated_model[0][1],
                                                         number_of_samples=number_of_samples)),

                                       post_index_result=
                                       lambda estimation, data_by_model_by_estimation:
                                       ongoing_summary(estimation,
        ...
        return \
            [
                [
                    ...
                    for mv, _ in enumerate(model_variants_and_queries_per_generated_model[0][0])
                ]
                for ns, _ in enumerate(numbers_of_samples)
            ]

    variance_by_model_by_number_of_samples = aggregate_data_over_estimations(torch.Tensor.var, 0)  # 0 = estimate
    mean_time_by_model_by_number_of_samples = aggregate_data_over_estimations(torch.Tensor.mean, 1)  # 1 = time
    ...
    for model_index in range(len(model_variants_and_queries_per_generated_model[0][0])):
        variance_by_number_of_samples = [variance_by_model[model_index]
                                         for variance_by_model in variance_by_model_by_number_of_samples]
        mean_time_by_number_of_samples = [mean_time_by_model[model_index]
                                          for mean_time_by_model in mean_time_by_model_by_number_of_samples]
        plt.plot(mean_time_by_number_of_samples, variance_by_number_of_samples, color=colors[model_index])
    plt.show()

    print("Finished at:", datetime.datetime.now().strftime("%Y-%mv-%d %H:%M:%S"))


def ongoing_summary(current_estimation, data_by_model_by_estimation, max_number_of_estimations_shown, models):
    if current_estimation > 0:
        variance_by_model = []
        for model_index, model in enumerate(models):
            print(f"{model.name:13}")
            estimates_so_far = [data_by_model_by_estimation[e][model_index][0] for e in range(current_estimation + 1)]
