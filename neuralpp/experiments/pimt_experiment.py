...
joint_simple = get_normal_piecewise_polynomial_approximation(x, mu1, 1.0, generator=mu1) \
               * \
               get_normal_piecewise_polynomial_approximation(mu1, BasicExpression.new_constant(0.0), 1.0, generator=mu1)
joint = get_normal_piecewise_polynomial_approximation(x, mu1, 1.0, generator=mu1) \
    * \
...
def two_normals_expectation():
    ...
    P_x_eq_0 = basic_integral(mu1, Z3SolverExpression.from_expression(mu1 > -20.0) & (mu1 < 20.0), joint_simple_x_eq_0)
    P_x_eq_0 = normalizer.normalize(P_x_eq_0, Z3SolverExpressionDummy())
    print(P_x_eq_0)
    P_mu1_given_x_eq_0 = joint_simple_x_eq_0 / P_x_eq_0
    E_mu1 = basic_integral(mu1, Z3SolverExpression.from_expression(mu1 > -20.0) & (mu1 < 20.0), mu1 * P_mu1_given_x_eq_0)
...
if __name__ == "__main__":
    ...
    evaluation_general("2 Normals-concrete x=1",
                       normalization_generator(lambda: basic_integral(mu1, Z3SolverExpression.from_expression(mu1 > -20.0) & (mu1 < 20.0), joint_simple.replace(x, BasicConstant(1.0)))),
                       {})
    evaluation_general("2 Normals-concrete x=0",
                       normalization_generator(lambda: basic_integral(mu1, Z3SolverExpression.from_expression(mu1 > -20.0) & (mu1 < 20.0), joint_simple.replace(x, BasicConstant(0.0)))),
                       {})
    evaluation_general("2 Normals",
                       normalization_generator(lambda: basic_integral(mu1, Z3SolverExpression.from_expression(mu1 > -20.0) & (mu1 < 20.0), joint_simple)),
                       {'x': 1.0})
    evaluation_general("Joint-concrete x",
                       normalization_generator(lambda: basic_integral(mu1, Z3SolverExpression.from_expression(mu1 > -20.0) & (mu1 < 20.0), joint.replace(x, BasicConstant(0.0)))),
                       {'mu2': 0.0})
    ...
    evaluation_general("Joint",
                       normalization_generator(lambda: basic_integral(mu1, Z3SolverExpression.from_expression(mu1 > -20.0) & (mu1 < 20.0), joint)),
                       {'x': 0.0, 'mu2': 0.0})
