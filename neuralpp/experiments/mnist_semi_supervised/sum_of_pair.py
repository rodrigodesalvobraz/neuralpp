...
def generate_chain_of_summands_digits_values_and_constraint_equal_to_sum(
    number_of_digits,
    ...
    d1_values = torch.cat(
        [torch.randint(max_d1_value[i], (1,)) for i in range(batch_size)]
    )
...
parameters.custom_digits_and_constraints_values_batches_generator = (
    generate_chain_of_summands_digits_values_and_constraint_equal_to_sum
)
