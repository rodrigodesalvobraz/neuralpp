...
from inference.graphical_model.representation.factor import PyTorchTableFactor
...
from inference.graphical_model.variable import DiscreteVariable
...
def test_sample(x, y, z, log_space, batch_size):
    ...
    for number_of_variables in range(1, 3):
        ...
        # TODO: we should be able to sample from unnormalized factors
        ...
        for batch_row in range(effective_batch_size):
            ...
            assert torch.allclose(probabilities, empirical_probabilities, atol=absolute_tolerance), \
                "Samples deviated from expected distribution; this is possible but should be an extremely rare event."
