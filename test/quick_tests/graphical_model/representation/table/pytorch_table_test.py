...
from inference.graphical_model.representation.table.pytorch_table import PyTorchTable, BatchCoordinatesDoNotAgreeException
from util import util
from util.util import check_that_exception_is_thrown, matrix_from_function
...
def batch_size1(request):
    return \
        -1 if request.param == "No batch 1" else \
            0 if request.param == "Empty batch 1" else \
                10
...
def batch_size2(request):
    return \
        -1 if request.param == "No batch 2" else \
            0 if request.param == "Empty batch 2" else \
                10
...
def batch_size_empty(request):
    return \
        -1 if request.param == "No batch empty" else \
            0 if request.param == "Empty batch empty" else \
                10
...
def table1(class_to_use, batch_size1):
    ...
    if batch_size1 == -1:
        ...
        table1 = class_to_use.from_function((batch_size1, 3, 2), (range(batch_size1), range(3), range(2)),
                                            f1_ixy, batch=True)
...
def table2(class_to_use, batch_size2):
    if batch_size2 == -1:
        ...
        table2 = class_to_use.from_function((batch_size2, 3), (range(batch_size2), range(3)),
...
def empty(class_to_use, batch_size_empty):
    if batch_size_empty == -1:
        ...
        empty = class_to_use.from_function((batch_size_empty, 0), (range(batch_size_empty), range(0)), lambda i: None,
                                           batch=True)
    print(f"empty: {empty}")
    return empty
...
def test_init_table1(table1, batch_size1):
    ...
    if batch_size1 == -1:
        ...
        table1_data = matrix_from_function([range(batch_size1), range(3), range(2)], f1_ixy)
    table1_shape = (3, 2)
    table1_length = 6
...
def run_init_test(batch_size, table, table_length, table_data, table_shape):
    if batch_size == -1:
        ...
        assert torch.allclose(table.potentials_tensor(),
...
def test_assignments1(table1):
    ...
    assert list(table1.assignments()) == [(0, 0), (0, 1), (1, 0), (1, 1), (2, 0), (2, 1)]
...
def run_expand_test(table, batch_size, class_to_use):
    for i in range(10):
        ...
        shape_to_insert = tuple(random.randrange(5) for i in range(shape_to_insert_length))
        dim = random.randrange(len(table.non_batch_shape))
        expected_table_expand = make_expected_expanded(table, shape_to_insert, dim, class_to_use, batch_size)
...
def make_expected_expanded(table, shape_to_insert, dim, class_to_use, batch_size):
    ...
    new_shape = batch_dim + table.non_batch_shape[:dim] + shape_to_insert + table.non_batch_shape[dim:]
    new_ranges = [range(d) for d in new_shape]

    def f(*args):
        effective_dim = dim if batch_size == -1 else dim + 1
        args_without_expansion = args[:effective_dim] + args[effective_dim + len(shape_to_insert):]
    ...
    expected_table_expand = class_to_use.from_function(new_shape, new_ranges, f, batch=(batch_size != -1))
...
def run_permute_test(table, batch_size, class_to_use):
    for i in range(10):
        ...
        permutation = random.sample(range(non_batch_number_of_dimensions), non_batch_number_of_dimensions)
        expected_table_permutation = make_expected_permutation(table, permutation, class_to_use, batch_size)
...
def make_expected_permutation(table, permutation, class_to_use, batch_size):
    ...
    def f(*args_in_permuted_order):
        args_in_original_order = tuple(args_in_permuted_order[i] for i in inverse_permutation)
    ...
    expected_table_permutation = class_to_use.from_function(permuted_shape, permuted_ranges, f,
                                                            batch=(batch_size != -1))
...
def test_non_batch_slice1(class_to_use, table1, batch_size1):
    ...
    run_non_batch_slice_tests_no_assumption_of_equal_batch_rows(
        table1, f1_ixy, table1.non_batch_shape, batch_size1, class_to_use, test_coordinates)


def run_non_batch_slice_tests_no_assumption_of_equal_batch_rows(
        table, function, original_non_batch_shape, batch_size, class_to_use, test_coordinates):
    ...
    for coordinates in test_coordinates:
        ...
        non_batch_shape = tuple(original_non_batch_shape[dim]
...
def test_non_batch_slice2(class_to_use, table2, batch_size2):
    ...
    run_non_batch_slice_tests_assuming_equal_batch_rows(table2, "table2", tests, batch_size2, class_to_use)
...
def test_non_batch_slice_empty(class_to_use, empty, batch_size_empty):
    ...
    run_non_batch_slice_tests_assuming_equal_batch_rows(empty, "empty", tests, batch_size_empty, class_to_use)


def run_non_batch_slice_tests_assuming_equal_batch_rows(table, table_name, tests, batch_size, class_to_use):
    """
    Tests slice without batch coordinates (multiple values for the same coordinate).
    """
...
def run_non_batch_slice_tests2(table, table_name, tests, batch_size, class_to_use):
    ...
    for coordinates, expected_non_batch_data_array in tests:
        actual = table.slice(coordinates)
        print(f"{table_name}.slice(): {actual}")
        expected = make_table_from_non_batch_data_array(class_to_use, batch_size, expected_non_batch_data_array)
        print("expected:", expected)
        assert actual == expected
...
def test_batch_slice1(class_to_use, table1, batch_size1):
    ...
    if batch_size1 == -1:
        ...
        for coordinates in [[1, []], [[], 1], [[], []], ]:
            expected = class_to_use.from_function(
                shape=(0,),  # only row dimension returns since non-batch dimensions would be determined.
        ...
        for coordinates in [[1, 1], [[0, 1]*(n//2), [0, 1]*(n//2)], [1, [1]*n], [[1]*n, 1]]:
            at_row = lambda row, c: c[row] if isinstance(c, list) else c
            expected = class_to_use.from_function(
                shape=(n,),  # only row dimension returns since non-batch dimensions would be determined.
                function_arguments_iterables=(range(n),),
                function_of_potentials=lambda i: f1_ixy(i, at_row(i, coordinates[0]), at_row(i, coordinates[1])),
                batch=True)
            actual = table1.slice(coordinates)
            assert actual == expected
    ...
    if batch_size1 == -1:
        coordinates = [[0, 1, 0], [0, 1]]  # batch coordinates with difference numbers of values
        check_that_exception_is_thrown(lambda: table1.slice(coordinates), BatchCoordinatesDoNotAgreeException)
    elif batch_size1 == 0:
        coordinates = [1, [0, 1]]
        ...
        n = table1.number_of_batch_rows()
        coordinates = [[1]*(n//2), [1]*n]  # coordinates not agreeing on number of values
        check_that_exception_is_thrown(lambda: table1.slice(coordinates), BatchCoordinatesDoNotAgreeException)
        coordinates = [[1]*(n//2), [1]*(n//2)]  # different from number of batch rows
        check_that_exception_is_thrown(lambda: table1.slice(coordinates), BatchCoordinatesDoNotAgreeException)
...
def test_mul(class_to_use):
    ...
    table1 = class_to_use.from_array([[1., 2.], [3., 4.]], batch=False)
    table2 = class_to_use.from_array([[10., 20.], [30., 40.]], batch=False)
    expected = class_to_use.from_array([[10., 40.], [90., 160.]], batch=False)
    run_mul_test(expected, table1, table2)

    table1 = class_to_use.from_array([[10., 20.], [30., 40.]], batch=False)
    table2 = class_to_use.from_array([[1., 2.], [3., 4.]], batch=False)
    expected = class_to_use.from_array([[10., 40.], [90., 160.]], batch=False)
    run_mul_test(expected, table1, table2)

    # Now we move to some one-batch cases

    table1 = class_to_use.from_array([1., 2.], batch=False)
    table2 = class_to_use.from_array([[10., 20.], [30., 40.]], batch=True)
    expected = class_to_use.from_array([[10., 40.], [30., 80.]], batch=True)
    run_mul_test(expected, table1, table2)

    table1 = class_to_use.from_array([[1., 2.], [3., 4.]], batch=True)
    table2 = class_to_use.from_array([10., 20.], batch=False)
    expected = class_to_use.from_array([[10., 40.], [30., 80.]], batch=True)
    run_mul_test(expected, table1, table2)
    ...
    table1 = class_to_use.from_array([[1., 2.], [3., 5.]], batch=True)
    table2 = class_to_use.from_array([[10., 20.], [30., 40.]], batch=True)
    expected = class_to_use.from_array([[10., 40.], [90., 200.]], batch=True)
    run_mul_test(expected, table1, table2)

    table1 = class_to_use.from_array([[10., 20.], [30., 40.]], batch=True)
    table2 = class_to_use.from_array([[1., 2.], [3., 5.]], batch=True)
    expected = class_to_use.from_array([[10., 40.], [90., 200.]], batch=True)
    run_mul_test(expected, table1, table2)
    ...
    try:
        ...
        raise AssertionError("Multiplication of two batches with different number of rows should have but did not.")
    except AssertionError:
        pass

    # This should also fail for the same reason
    try:
        table1 = class_to_use.from_array([[1., 2.]], batch=True)
        table2 = class_to_use.from_array([[1., 2.], [3., 5.]], batch=True)
        expected = None
        actual = table1 * table2
        raise AssertionError("Multiplication of two batches with different number of rows should have but did not.")
...
def test_sum_out1(class_to_use, table1, batch_size1):
    ...
    if batch_size1 == -1:
        ...
        actual = table1.sum_out(0)
        expected = class_to_use.from_array([[6.0*10**i, 9.0*10**i] for i in range(batch_size1)], batch=True)
        assert actual == expected

        actual = table1.sum_out(1)
        expected = class_to_use.from_array([[1.0*10**i, 5.0*10**i, 9.0*10**i] for i in range(batch_size1)], batch=True)
        assert actual == expected

        actual = table1.sum_out((0, 1))
        expected = class_to_use.from_array([15.0*10**i for i in range(batch_size1)], batch=True)
        assert actual == expected
...
def test_normalize1(class_to_use, table1, batch_size1):
    if batch_size1 == -1:
        ...
        print(actual)
        expected = class_to_use.from_array([[0.0, 0.0667], [0.1333, 0.2], [0.2667, 0.3333]], batch=False)
        assert actual == expected
        ...
        actual = table1.normalize()
        print(actual)
        expected = class_to_use.from_array([[[0.0, 0.0667], [0.1333, 0.2], [0.2667, 0.3333]]]*batch_size1, batch=True)
        assert actual == expected
...
def make_table_from_row_array(class_to_use, batch_size, expected_row_array):
    batch = batch_size != -1
    if batch:
        # here we must create the table from a tensor which, unlike arrays, is able to represent a shape
        # even if the batch is empty (creating a table from an empty array renders its shape equal to (0)).
        non_batch_shape = util.array_shape(expected_row_array)
        total_shape = (batch_size, *non_batch_shape)
        expected_tensor = torch.tensor([expected_row_array] * batch_size).reshape(total_shape)
...
def make_table_from_non_batch_data_array(class_to_use, batch_size, expected_non_batch_data_array):
    batch = batch_size != -1
    if batch:
        non_batch_shape = util.array_shape(expected_non_batch_data_array)
        total_shape = (batch_size, *non_batch_shape)
        expected_tensor = torch.tensor([expected_non_batch_data_array] * batch_size).reshape(total_shape)
        expected = class_to_use.from_array(expected_tensor, batch)
    else:
