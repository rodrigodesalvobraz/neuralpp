...
from inference.graphical_model.brute_force import BruteForce
from inference.graphical_model.variable.integer_variable import IntegerVariable
from inference.graphical_model.variable_elimination import VariableElimination
...
def test_variable_elimination():
    ...
    for log_space in {False, True}:
        ...
        model = [
            ...
            PyTorchTableFactor([], torch.ones(()), log_space=log_space),  # just to test the edge case

            PyTorchTableFactor([x], [0.4, 0.1, 0.5], log_space=log_space),
            PyTorchTableFactor.from_function([y, x], lambda y, x: 0.5 if x == 2 else 1.0 if y == x else 0.0, log_space=log_space),
            PyTorchTableFactor.from_function([z, y], lambda z, y: 1.0 if z == y else 0.0, log_space=log_space),
        ]
    ...
    for i in range(10):
        model = generate_model(number_of_factors=15, number_of_variables=8, cardinality=2)
