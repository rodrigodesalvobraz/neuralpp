...
from util.util import merge_dicts


def check_and_show_conditional_distributions(neural_factor, ground_truth=None, tolerance=0.05):
    """
    ...
    for input_assignment_dict in DiscreteVariable.assignments_product_dicts(input_variables):

        potentials = [compute_potential(neural_factor, input_assignment_dict, output_assignment_dict)
                      for output_assignment_dict in DiscreteVariable.assignments_product_dicts([output_variable])]
        assert abs(sum(potentials) - 1) < 0.0001

        for output_assignment_dict in DiscreteVariable.assignments_product_dicts([output_variable]):
            potential, assignment_dict = \
                compute_potential_and_assignment(neural_factor, input_assignment_dict, output_assignment_dict)
            print("----------------------")
            print(f"Neural factor potential for {assignment_dict}:", util.rounded_list(potential), end="")
