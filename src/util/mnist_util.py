...
from util.util import get_or_put
...
def read_mnist(max_datapoints=None):
    return pickle_cache(lambda: read_mnist_no_cache(max_datapoints),
...
def read_mnist_no_cache(max_datapoints=None):
    ...
    images_by_digits = {}
    for p in phases:
        mnist_dataset = datasets.MNIST('../data', train=(p == "train"), download=True, transform=transform)
        ...
        for image_singleton_batch, digit in tqdm(loader[p]):
            image = image_singleton_batch[0][0]  # get single row of batch and single color
...
def show_images_and_labels(n_rows, n_cols, images, labels):
    ...
    for i, (image, label) in enumerate(islice(zip(images, labels), n_positions), start=1):
