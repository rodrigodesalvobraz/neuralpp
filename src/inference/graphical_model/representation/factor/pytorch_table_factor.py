from inference.graphical_model.representation.factor.table_factor import TableFactor
from inference.graphical_model.representation.table.pytorch_log_table import PyTorchLogTable
from inference.graphical_model.representation.table.pytorch_table import PyTorchTable
from inference.graphical_model.representation.table.table_util import shape
...
class PyTorchTableFactor(TableFactor):
    ...
    def __init__(self, variables, array_or_table_of_potentials, log_space=True, batch=False):
        super().__init__(
            variables,
            array_or_table_of_potentials if isinstance(array_or_table_of_potentials, PyTorchTable)
            else PyTorchLogTable.from_array(array_or_table_of_potentials, batch=batch) if log_space
            else PyTorchTable.from_array(array_or_table_of_potentials, batch=batch)
        )
        assert self.table.non_batch_shape == (variables_shape := tuple(v.cardinality for v in variables)), \
    ...
    @staticmethod
    def from_function_and_constructor(variables, function, constructor, log_space=True, batch_size=None):
        """
        Builds a PyTorchTableFactor from its variables, a function and a constructor.
    ...
    def from_function(variables, function, log_space=True, batch_size=None):
        """
        Builds a PyTorchTableFactor from its variables and a function.
        If batch_size is None, creates a non-batch factor.
        If batch_size is an integer, creates a batch factor; in this case, first parameters of function must be
        the batch row index.
        """
        return \
            PyTorchTableFactor.from_function_and_constructor(
                variables, function, PyTorchTableFactor, log_space=log_space, batch_size=batch_size)

    @staticmethod
    def from_predicate(variables, predicate, log_space=True, batch_size=None):
        return PyTorchTableFactor.from_function(variables, lambda *args: float(predicate(*args)), log_space, batch_size)
