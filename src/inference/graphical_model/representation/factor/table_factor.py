from inference.graphical_model.representation.factor.atomic_factor import AtomicFactor
from inference.graphical_model.representation.table.table_util import shape, index_of, permutation_from_to
from inference.graphical_model.variable.discrete_variable import DiscreteVariable
from util.discrete_sampling import discrete_sample
...
class TableFactor(AtomicFactor):
    ...
    def condition_on_non_empty_dict(self, assignment_dict):
        non_batch_slice_coordinates = tuple(assignment_dict[v] if v in assignment_dict else slice(None)
                                            for v in self.variables)
        non_conditioned_variables = [v for v in self.variables if v not in assignment_dict or isinstance(assignment_dict[v], slice)]
    ...
    def mul_by_non_identity(self, other):
        ...
        f1_e1_common_table, e1 = self.get_permuted_table_with_selected_variables_moved_to_the_end(self, common)
        f2_e2_common_table, e2 = self.get_permuted_table_with_selected_variables_moved_to_the_end(other, common)

        result_variables = e1 + e2 + common

        f1p_table = f1_e1_common_table.expand(shape_to_be_inserted=shape(e2), dim=len(e1))
    ...
    @staticmethod
    def get_permuted_table_with_selected_variables_moved_to_the_end(factor, selected_variables):
        ...
        permuted_table = factor.table.permute(index_of(variables_in_desired_order, factor.variables))
    ...
    def argmax(self):
        ...
        if len(self.variables) == 1:  # if there is a single variable, indices is 1D
            assignment_getter = lambda var_index: indices  # regardless of being batch or not
        else:
            assignment_getter = lambda var_index: indices[:, var_index] if self.batch else indices[var_index]
        assignment_dict = {v: assignment_getter(var_index) for var_index, v in enumerate(self.variables)}
    ...
    def single_sample(self):
        if len(self.table) == 0:
            ...
            if abs(entries_sum - 1.0) > 0.00001:
                raise Exception(f"Sampled factor is not normalized. Sum of entries is {entries_sum}")
            return discrete_sample(self.assignments(), lambda assignment: self.table[assignment])
    ...
    def __eq__(self, other):
        ...
        if isinstance(other, TableFactor):
            if self.variables == other.variables:
                ...
                return self.table == other.get_table_permuted_to_agree_with_table_of(self)
            else:
                return False
        else:
            raise Exception(f"Comparison of PyTorchFactor to factors other than PyTorchFactor is not implemented. "
                            f"Got {type(other)}")
