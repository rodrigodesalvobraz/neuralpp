from inference.graphical_model.representation.frame.dict_data_loader import DictDataLoader
from inference.graphical_model.representation.frame.dict_frame import is_frame, generalized_len_of_dict_frame
...
class TupleDataLoader:
    ...
    def __init__(self, dataset_tuple, component_data_loader_maker):
        ...
        self.data_loaders = [component_data_loader_maker(ds) for ds in self.dataset_tuple]
...
class DataLoadersMustHaveTheSameLength(BaseException):
    ...
    def __init__(self):
        super(DataLoadersMustHaveTheSameLength, self).__init__("Data loaders must have the same length")
...
def get_data_loader(observation_frame, query_assignment_frame, batch_size=100):
    ...
    if use_batches:
        ...
        frame_data_loader_maker = lambda frame: DictDataLoader(frame, batch_size=batch_size)
        data_loader = TupleDataLoader((observation_frame, query_assignment_frame), frame_data_loader_maker)
    else:
